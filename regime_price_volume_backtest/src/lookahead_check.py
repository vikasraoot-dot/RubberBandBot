...
def check_regime(n_points: int, rng: np.random.Generator) -> dict:
    """Same test for the SPY trend block and the VIX percentile."""
    ...
    for i in pts:
        ...
        ok_v = np.isclose(full_p.loc[d], vp, equal_nan=True)
        if not (ok_t and ok_v):
            bad += 1
            log.error("regime look-ahead mismatch at %s", d.date())
    return {"ticker": "REGIME(SPY,VIX)", "points": int(len(pts)), "mismatches": bad}
